...
static PyObject *settings_new(PyTypeObject *subtype, PyObject *args, PyObject *keywords) {
    ...
    const char *const error = PARSE_TUPLE(args, "lfffI", &self->settings.convention, &self->settings.gain, &self->settings.accelerationRejection, &self->settings.magneticRejection, &self->settings.rejectionTimeout);
    if (error != NULL) {
